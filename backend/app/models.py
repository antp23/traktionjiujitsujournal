"""ORM models. Table and column names are frozen: the production SQLite file
predates this rebuild and must keep working without a migration."""
import uuid
...
class Session(Base):
    ...
    session_techniques = relationship(
        "SessionTechnique", back_populates="session", cascade="all, delete-orphan"
    )
...
class Technique(Base):
    ...
    gi_nogi = Column(String, default="both")   # gi, no-gi, both (legacy rows: no_gi)
    ...
    links_from = relationship(
        "LinkedTechnique", foreign_keys="LinkedTechnique.from_technique_id",
        back_populates="from_technique", cascade="all, delete-orphan",
    )
    links_to = relationship(
        "LinkedTechnique", foreign_keys="LinkedTechnique.to_technique_id",
        back_populates="to_technique", cascade="all, delete-orphan",
    )
...
class LinkedTechnique(Base):
    ...
    from_technique = relationship(
        "Technique", foreign_keys=[from_technique_id], back_populates="links_from"
    )
    to_technique = relationship(
        "Technique", foreign_keys=[to_technique_id], back_populates="links_to"
    )
...
class Coach(Base):
    ...
    coach_id = Column(String, primary_key=True, default=new_uuid)
    name = Column(String, nullable=False)
...
class User(Base):
    ...
    whatsapp_identity = relationship(
        "WhatsAppIdentity", back_populates="user", uselist=False, cascade="all, delete-orphan"
    )
...
class AuthToken(Base):
    ...
    token_type = Column(String, nullable=False, default="magic_link")  # magic_link | session
...
class Membership(Base):
    ...
    role = Column(String, nullable=False, default="athlete")  # owner | coach | athlete
...
class ShareThread(Base):
    ...
    source_type = Column(String, nullable=False)  # goal | note
